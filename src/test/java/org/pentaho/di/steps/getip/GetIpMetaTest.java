...
import org.pentaho.di.steps.getip.GetIpMeta;
...
public class GetIpMetaTest {
    ...
    public void testGetStepData() {
        GetIpMeta m = new GetIpMeta();
        assertEquals(GetIpMeta.class, m.getStepData().getClass());
    }
    ...
    public void testStepAnnotations() {
        ...
        Step stepAnnotation = GetIpMeta.class.getAnnotation(Step.class);
        ...
        assertEquals(GetIpMeta.class.getPackage().getName(), stepAnnotation.i18nPackageName());
        ...
        InjectionSupported injectSupportAnnotation = GetIpMeta.class.getAnnotation(InjectionSupported.class);
        ...
        for (Field field : GetIpMeta.class.getDeclaredFields()) {
            if (field.isAnnotationPresent(Injection.class) || field.isAnnotationPresent(InjectionDeep.class)) {
                hasInjection = true;
            }
        }
        for (Method method : GetIpMeta.class.getDeclaredMethods()) {
    ...
    public void testDefaults() throws KettleStepException {
        ...
        assertEquals(rowMeta.getFieldNames()[0], m.getParamIp());
    }
     ...
     * These are typically custom implementations of the FieldLoadSaveValidator interface for each Object type.
         */
        LoadSaveTester<GetIpMeta> tester
                = new LoadSaveTester<GetIpMeta>(GetIpMeta.class, attributes);
    ...
    public void testChecks() {
        ...
        for (CheckResultInterface result : checkResults) {
            if (result.getType() == CheckResultInterface.TYPE_RESULT_ERROR
                    && result.getText().equals(BaseMessages.getString(GetIpMeta.class, "Demo.CheckResult.ReceivingRows.ERROR"))) {
                foundMatch = true;
            }
        }
        ...
        for (CheckResultInterface result : checkResults) {
            if (result.getType() == CheckResultInterface.TYPE_RESULT_OK
                    && result.getText().equals(BaseMessages.getString(GetIpMeta.class, "Demo.CheckResult.ReceivingRows.OK"))) {
                foundMatch = true;
            }
        }
        assertTrue("The step checks should fail if no input fields are given", foundMatch);
